...
rm -rf "$OUTPUT_DIR/uftrace.data"
rm -rf "$TRACE_PATH/../kernel"
...
lttng enable-event --userspace lttng_ust_libc:free
lttng enable-event --userspace lttng_ust_libc:calloc
lttng enable-event --userspace lttng_ust_libc:realloc
lttng enable-event --userspace lttng_ust_libc:posix_memalign
lttng enable-event --userspace lttng_ust_libc:aligned_alloc
lttng enable-event --userspace lttng_ust_libc:memalign
...
echo "Adding context fields..."
lttng add-context --userspace --type vtid --type procname
lttng add-context --kernel --type vpid --type procname
...
eval "LD_PRELOAD=\"liblttng-ust-libc-wrapper.so\" uftrace record --force --no-libcall -d $OUTPUT_DIR/uftrace.data $PROGRAM_CMD"
